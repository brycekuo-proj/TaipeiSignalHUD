...
> v0.0.6-roadtest 針對道路實測發現的 1–2 秒顯示延遲，將 MCP 拉取頻率提高到約 0.45 秒、Overlay 刷新提高到 0.25 秒，並依實測 HTTP round-trip time 補償估算單程網路延遲。這只消除傳輸/顯示延遲，不會預先切換號誌相位；若仍有固定偏差，交由 MCP consistency event 做路口 cycle/offset 校正。
